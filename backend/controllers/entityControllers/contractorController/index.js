...
class ContractorsController {
  ...
  static async findOne(req, res) {
    try {
      const { id } = req.params;
      const data = await ContractorsModel.findOne(id);
      if (!data) {
      ...
      console.error(`Error fetching contractor with ID ${req.params.id}:`, error);
      return res.status(500).send({ status: false, msg: 'Internal Server Error' });
    }
  }
  ...
  static async create(req, res) {
    try {
      ...
      const contractorId = await ContractorsModel.create(con_name, con_contact, con_alt_contact, con_address, con_email);
      return res.status(201).send({ status: true, msg: 'Contractor added successfully', data: { con_id: contractorId } });
    } catch (error) {
  ...
  static async update(req, res) {
    try {
      const { id } = req.params;
      const { con_name, con_contact, con_alt_contact, con_address, con_email } = req.body;
      const updated = await ContractorsModel.update(id, con_name, con_contact, con_alt_contact, con_address, con_email);
      ...
      console.error(`Error updating contractor with ID ${req.params.id}:`, error);
      return res.status(500).send({ status: false, msg: 'Internal Server Error' });
    }
  }
  ...
  static async remove(req, res) {
    try {
      ...
      console.error(`Error deleting contractor with ID ${req.params.id}:`, error);
      return res.status(500).send({ status: false, msg: 'Internal Server Error' });
    }
  }
