...
class SuperviserController {
   ...
   static async findOne(req, res) {
      try {
         ...
         console.error(`Error fetching superviser with ID ${req.body.id}:`, error);
   ...
   static async create(req, res) {
      try {
         ...
         if (!sup_name || !sup_contact)
            return res.status(400).send({ status: false, msg: 'Name and contact are required' });
   ...
   static async update(req, res) {
      try {
         ...
         console.error(`Error updating superviser with ID ${req.body.id}:`, error);
   ...
   static async remove(req, res) {
      try {
         ...
         console.error(`Error deleting superviser with ID ${req.body.id}:`, error);
