...
const ClientsCoreModel = require('@/models/coreEntityModels/clientModel');
...
class ClientsController {
  ...
  static async create(req, res) {
    try {
      const { client_name, client_ref_no, client_contact, client_alt_contact, client_address, client_email } =
        req.body;
      if (!client_name || !client_contact) {
        return res.status(400).send({ status: false, msg: 'Missing required fields' });
      }
      const [data] = await ClientsCoreModel.getLastClientRef();
      let newRef;
      if (data) {
        let lastNum = parseInt(data['client_ref_no'].slice(-4));
        newRef = data['client_ref_no'].replace(lastNum, lastNum + 1);
      } else { newRef = 'JGCC0001'; }
