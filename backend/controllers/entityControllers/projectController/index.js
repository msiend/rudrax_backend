...
class ProjectController {
   ...
   static async create(req, res) {
      try {
         ...
         res.status(201).send({ status: true, msg: 'Project created successfully', data: {
   ...
   static async update(req, res) {
      ...
      try {
         ...
         res.status(200).send({ status: true, msg: 'Project updated successfully', data: {
             pro_id: pro_id ,
             pro_client_r_id:pro_client_r_id,
             pro_name: pro_name,
   ...
   static async remove(req, res) {
      const { pro_id } = req.body;
      try {
         const deleted = await ProjectModel.remove(pro_id);
         if (!deleted) {
            return res.status(404).send({ status: false, msg: 'Project not found', data: null });
         }
         res.status(200).send({ status: true, msg: 'Project deleted successfully', data: { pro_id } });
      } catch (error) {
