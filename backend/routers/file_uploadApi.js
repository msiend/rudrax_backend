...
coreRouter.post('/core/file_manager/upload/file', FS_File_System_Files, async (req, res) => {
   try {
      ...
      const filePath = req.file.path.replace(/\\/g, '/'); 
      const file_name = req.file.originalname.replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z ]/g, '').trim().substring(0, 20);
      const insertedId = await file_managerDocsModel.create(pro_r_id, filePath,req.query.type||'doc_file',file_name);
      return res.status(200).json({
         status: true,
         msg: 'Project File uploaded and path stored successfully',
         filePath,
...
coreRouter.post('/core/file_manager/upload/image', FS_File_System_Images, async (req, res) => {
   try {
      ...
      const imagePath = req.file.path.replace(/\\/g, '/');
      const file_name = req.file.originalname.replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z ]/g, '').trim().substring(0, 20);
      const insertedId = await file_managerDocsModel.create(pro_r_id, imagePath,req.query.type||'doc_image',file_name);
      return res.status(200).json({
         status: true,
         msg: 'Project Image uploaded and stored successfully',
         imagePath,
...
coreRouter.delete('/core/file_manager/delete/:fs_doc_id', async (req, res) => {
   try {
      ...
      if (deleted) {return res.status(200).json({status: true,msg: 'file_manager document and file deleted successfully',filePath,});
      } else {
         return res.status(404).json({ status: false, msg: 'Document not found or already deleted' });
      }
   } catch (error) {
