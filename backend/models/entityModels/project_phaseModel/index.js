...
class ProjectPhaseModel {
  ...
  static async create(data) {
    const query = 'INSERT INTO project_phase (pro_phase_id, pro_id, pro_phase_status, pro_phase_deadline) VALUES (?, ?, ?, ?)';
    const conn = await pool.getConnection();
    try {
      const [result] = await conn.query(query, [
        data.pro_phase_id, data.pro_id, data.pro_phase_status, data.pro_phase_deadline
      ]);
  ...
  static async update(pro_phase_id, data) {
    const query = 'UPDATE project_phase SET pro_id = ?, pro_phase_status = ?, pro_phase_deadline = ? WHERE pro_phase_id = ?';
    const conn = await pool.getConnection();
    try {
      const [result] = await conn.query(query, [
        data.pro_id, data.pro_phase_status, data.pro_phase_deadline, pro_phase_id
      ]);
