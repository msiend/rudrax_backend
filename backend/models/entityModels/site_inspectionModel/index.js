...
class SiteInspectionModel {
   constructor(project_id, si_date, si_location, si_type, status) {
   ...
   static async create(project_id, si_date, si_location, si_type, status) {
      const connPool = await pool.getConnection();
      const insertSQL = `
      INSERT INTO site_inspections 
        (project_id, si_date, si_location, si_type, status) 
      VALUES (?, ?, ?, ?, ?);
    `;

      try {
         const [result] = await connPool.query(insertSQL, [project_id, si_date, si_location, si_type, status]);
   ...
   static async update(si_id, project_id, si_date, si_location, si_type, status) {
      const connPool = await pool.getConnection();
      const updateSQL = `
      UPDATE site_inspections
      SET project_id = ?, si_date = ?, si_location = ?, si_type = ?, status = ?
      WHERE si_id = ?;
    `;

      try {
         const [result] = await connPool.query(updateSQL, [project_id, si_date, si_location, si_type, status, si_id]);
